...
template <typename Key, typename Index, typename T, typename Allocator> class trie_const_iterator;
...
private:
  ...
  template <typename KeyT, typename TT, typename AllocatorT>
  friend class trie;
...
public:
  ...
  explicit trie_iterator(trie<Key,T,Allocator> *trie, trie_node<Key, Index, value_type> *n) : _trie(trie), _node{n} {}

  trie_iterator(const trie_iterator &other) : _trie(other._trie), _node(other._node) {}
  ...
  self &operator--() {
    ...
    while (_node->_parent != nullptr) {
      auto it = std::make_reverse_iterator(_node->_parent->_children.find(_node->_index));
  ...
  bool operator==(const self &other) const { return _trie == other._trie && _node == other._node; }
...
private:
  ...
  using mapped_type = T;
  using allocator_type = Allocator;
  using self = trie_const_iterator<key_type, index_type, mapped_type, allocator_type>;
...
public:
  explicit trie_impl(const Allocator &a = Allocator()) : Allocator(a), _root(new tree_node()), _size(0) {}

  trie_impl(const trie_impl &other) : Allocator(other), _root(new tree_node()), _size(other._size) {
    _root = other._root->clone(*static_cast<Allocator *>(this));
  }

  trie_impl(trie_impl &&other) : Allocator(other), _root(other._root), _size(other._size) {
    other._root = nullptr;
    other._size = 0;
  }
  ...
  std::pair<tree_node *, bool> insert(const key_type &key,
    ...
    for (const auto character : key) {
      ...
      if (current_node == nullptr) {
        current_node = parent_node->_children[character] = new tree_node(parent_node, character);
      }
    }
    if (current_node->_value != nullptr) {
  ...
  void erase(tree_node *node) {
    ...
    while (parent->_parent != nullptr && parent->_children.size() == 1 && parent->_value == nullptr) {
      child = parent;
...
struct trie_node {
  ...
  explicit trie_node(trie_node *parent = nullptr, const index_type key = index_type(), value_type *value = nullptr)
  ...
  void set_value(AllocatorT alloc, Args &&...args) {
    ...
    std::allocator_traits<AllocatorT>::construct(alloc, _value, std::forward<Args>(args)...);
  }
  ...
  trie_node *leftmostfirst() {
    return const_cast<trie_node *>(const_cast<const trie_node *>(this)->leftmostfirst());
  }
  ...
  trie_node *rightmostfirst() {
    return const_cast<trie_node *>(const_cast<const trie_node *>(this)->rightmostfirst());
  }
...
public:
  ...
  using iterator = trie_iterator<key_type, index_type, mapped_type, allocator_type>;
  using const_iterator = trie_const_iterator<key_type, index_type, mapped_type, allocator_type>;
...
private:
  using impl_type =
      trie_impl<key_type, mapped_type, allocator_type>;
...
public:
  ...
  trie(std::initializer_list<value_type> il, const allocator_type &a = allocator_type()) : _impl(a) {
  ...
  trie(FwdIterT first, FwdIterT last, const allocator_type &a = allocator_type()) : _impl(a) {
  ...
  bool operator<(const self &other) const {
    ...
    return std::lexicographical_compare(begin(), end(), other.begin(), other.end(), cmp);
  }
